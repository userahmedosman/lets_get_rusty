...
fn get_credit_card_info( cd:&HashMap<&str, &str>, name:&str) -> Result<CreditCardInfo, String>{
    let get_card_string = cd.get(name).ok_or(format!{"no card data found with the given name: {name}"})?;
